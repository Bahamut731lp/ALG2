...
public class PiskvorkyException extends RuntimeException {
    ...
    public PiskvorkyException(String message, int errCode, String... dataInfo) {
    	super(formatMessage(message, errCode, dataInfo));
    ...
    public static String formatMessage(String message, int errCode, String[] dataInfo) {
    	StringBuilder msg = new StringBuilder();
    	
    	msg.append(String.format("Chyba %s: %s%n", errCode, message));
    	
    	for (int i = 0; i < dataInfo.length; i += 2)
			msg.append(String.format("%s: %s%n", dataInfo[i], dataInfo[i + 1]));
